import type { Conversation, ConversationMode, AccessPolicy, Message, Run } from "@nousarium/contracts";
...
export function createSqliteStore(runtimePath: string): ConversationStore {
  ...
  return {
    ...
    async createConversation(input) {
      ...
      db.prepare(
        `INSERT INTO conversations (id, title, cursor_agent_id, mode, access_policy, pending_mode, pending_access_policy, journal_path, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      ).run(
        conversation.id,
        conversation.title,
    ...
    async updateConversation(id, patch) {
      ...
      db.prepare(
        `UPDATE conversations SET title=?, cursor_agent_id=?, mode=?, access_policy=?, pending_mode=?, pending_access_policy=?, journal_path=?, updated_at=? WHERE id=?`,
      ).run(
        next.title,
        next.cursorAgentId,
        next.mode,
    ...
    async createRun(run) {
      ...
      db.prepare(
        `INSERT INTO runs (id, conversation_id, status, mode, access_policy, git_before, git_after, error, started_at, finished_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      ).run(
        row.id,
        row.conversationId,
